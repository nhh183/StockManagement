    ...
    throws ServletException, IOException {
        ...
        try {
           ...
           if(loginUser==null || !"AD".equals(loginUser.getRoleID())){
               request.setAttribute("MGS", "Only admin can update transaction status");
               request.getRequestDispatcher("transactionList.jsp").forward(request, response);
               return;
           }
           
           int transactionID =Integer.parseInt(request.getParameter("transactionID"));
           String status=request.getParameter("stastus");
           
           TransactionDAO dao=new TransactionDAO();
           if(dao.updateTransaction(transactionID, status)){
