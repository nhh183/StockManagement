...
public class DeleteTransactionController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (transaction == null) {
                session.setAttribute("ERROR", "Transaction not found.");
            } else {
                if (!transaction.getUserID().equals(loginUser.getUserID()) && !loginUser.getRoleID().equals("AD")) {
                    session.setAttribute("ERROR", "You do not have permission to delete this transaction.");
                } else if (dao.deleteTransaction(id)) {
                    session.setAttribute("MSG", "Transaction deleted successfully.");
                } else {
                    session.setAttribute("ERROR", "Failed to delete the transaction.");
                }
            }
